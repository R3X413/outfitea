...
export function OutfitDisplay({ outfit }: OutfitDisplayProps) {
  ...
  const hasDress = outfit.items.some((item) => ["vestido", "mono"].includes(item.category.toLowerCase()))

  const topItem = outfit.items.find((item) =>
    ["camiseta", "camisa", "blusa", "suéter", "chaqueta"].includes(item.category.toLowerCase()),
  )

  const bottomItem = outfit.items.find((item) =>
    ["pantalón", "falda", "shorts", "jeans"].includes(item.category.toLowerCase()),
  )

  const dressItem = outfit.items.find((item) => ["vestido", "mono"].includes(item.category.toLowerCase()))
